...
| ``` # Chat Completions - 需要自己实现工具 # 这里展示调用外部搜索 API 的示例 curl https://api.example.com/search \\ -G \\ --data-urlencode "q=法国现任总统" \\ --data-urlencode "key=$SEARCH_API_KEY" ``` | ``` # Responses API - 直接使用内置工具 curl -X POST https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/responses \\ -H "Authorization: Bearer $DASHSCOPE_API_KEY" \\ -H "Content-Type: application/json" \\ -d '{ "model": "qwen3.7-plus", "input": "法国现任总统是谁？", "tools": [{"type": "web_search"}] }' ``` |
| --- | --- |
